...
2. VisibleHand assumes all disposable media in its Platform may contain ePHI, so it treats all disposable media with the same protections and disposal policies.
3. All destruction/disposal of ePHI media will be done in accordance with federal and state laws and regulations and pursuant to the VisibleHand's written retention policy/schedule. Records that have satisfied the period of retention will be destroyed/disposed of in an appropriate manner.
...
9. In the case of a VisibleHand Customer terminating a contract with VisibleHand and no longer utilizing VisibleHand Services, the following actions will be taken depending on the VisibleHand Services in use. In all cases it is solely the responsibility of the VisibleHand Customer to maintain the safeguards required of HIPAA once the data is transmitted out of VisibleHand Systems.
    * In the case of a Client termination, VisibleHand will provide the customer with the ability to export data in commonly used format, currently CSV, for 30 days from the time of termination.
    * In the case of a User termination, VisibleHand will provide the customer with 30 days from the date of termination to export data.
